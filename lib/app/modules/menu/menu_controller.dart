...
import 'package:flutter_tem_horario/app/modules/menu/profile/profile_page.dart';
...
abstract class _MenuBase with Store {
  ...
  _MenuBase(){
    ...
    this.menus.add(BottomNavigationBarModel(screenId: 1, icon: Icons.search,
        label: CustomLocalizations.of(navigatorKey.currentContext).translate("Procurar"), getPage: ()=>SearchPage()));

    this.menus.add(BottomNavigationBarModel(screenId: 2, icon: Icons.calendar_today, 
      	label: CustomLocalizations.of(navigatorKey.currentContext).translate("Agenda"), getPage: ()=>SchedulePage()));
    this.menus.add(BottomNavigationBarModel(screenId: 3, icon: Icons.person,
        label: CustomLocalizations.of(navigatorKey.currentContext).translate("Perfil"), getPage: ()=>ProfilePage()));
